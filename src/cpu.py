...
class CPU:
    ...
    def rl_prefix(self, reg):
        carry = (self.get_reg(reg) >> 7) & 0x01
        self.set_reg(reg, ((self.get_reg(reg) << 1) & 0xFF) | (self.get_flag(self.FLAG_C) & 0x01))
        self.set_flag(self.FLAG_Z, self.get_reg(reg) == 0)
        self.set_flag(self.FLAG_N, False)
        self.set_flag(self.FLAG_H, False)
        self.set_flag(self.FLAG_C, carry)

    def rr_prefix(self, reg):
        carry = self.get_reg(reg) & 0x01
        self.set_reg(reg, ((self.get_flag(self.FLAG_C) << 7) | (self.get_reg(reg) >> 1)) & 0xFF)
        self.set_flag(self.FLAG_Z, self.get_reg(reg) == 0)
        self.set_flag(self.FLAG_N, False)
        self.set_flag(self.FLAG_H, False)
        self.set_flag(self.FLAG_C, carry)

    def sla_prefix(self, reg):
        carry = (self.get_reg(reg) >> 7) & 0x01
        self.set_reg(reg, (self.get_reg(reg) << 1) & 0xFF)
        self.set_flag(self.FLAG_Z, self.get_reg(reg) == 0)
        self.set_flag(self.FLAG_N, False)
        self.set_flag(self.FLAG_H, False)
        self.set_flag(self.FLAG_C, carry)

    def sra_prefix(self, reg):
        carry = value & 0x01
        self.set_reg(reg, (self.get_reg(reg) >> 1) | (self.get_reg(reg) & 0b1000_0000))
        self.set_flag(self.FLAG_Z, self.get_reg(reg) == 0)
        self.set_flag(self.FLAG_N, False)
        self.set_flag(self.FLAG_H, False)
        self.set_flag(self.FLAG_C, carry)

    def swap_prefix(self, reg):
        self.set_reg((self.get_reg(reg) & 0x0F) << 4) | ((self.get_reg(reg) & 0xF0) >> 4)
        self.set_flag(self.FLAG_Z, self.get_reg(reg) == 0)
        self.set_flag(self.FLAG_N, False)
        self.set_flag(self.FLAG_H, False)
        self.set_flag(self.FLAG_C, False)

    def srl_prefix(self, reg):
    ...
    def bit_prefix(self, reg, bit):
        temp = (selg.get_reg(reg) & 2**bit) > 0
        self.set_flag(self.FLAG_Z, temp)
        self.set_flag(self.FLAG_N, False)
        self.set_flag(self.FLAG_H, False)

    def res_prefix(self, reg, bit):
        self.set_reg(reg, self.get_reg(reg) & ~(1 << bit))

    def set_prefix(self, reg, bit):
        self.set_reg(reg, self.get_reg(reg) | (1 << bit))
    ...
    def execute_instruction(self, opcode: int) -> int:
        ...
        if opcode == 0x00:   # NOP         | ---- | 1 4
            ...
            self.prefix = self.read_byte()
            self.execute_prefix(self.prefix)
            #cycles(4) ????? PyBoy no bich 6_6))) meme
            ...
            temp = self.read_byte()
            self.a = self.memory.read_byte(0xFF00 + temp)
            cycles(12)
    ...
    def execute_prefix(self, prefix) -> int:
        ...
        if prefix == 0x00: # RLC B       | Z00C | 2 8
            ...
            self.set_prefix("D", 2)
            cycles(8)
        elif prefix == 0xD3: # SET 2,E     | ---- | 2 8
            self.set_prefix("E", 2)
            cycles(8)
        elif prefix == 0xD4: # SET 2,H     | ---- | 2 8
            self.set_prefix("H", 2)
            cycles(8)
        elif prefix == 0xD5: # SET 2,L     | ---- | 2 8
            self.set_prefix("L", 2)
            cycles(8)
        elif prefix == 0xD6: # SET 2,(HL)  | ---- | 2 16
            self.set_prefix("(HL)", 2)
            cycles(16)
        elif prefix == 0xD7: # SET 2,A     | ---- | 2 8
            self.set_prefix("A", 2)
            cycles(8)
        elif prefix == 0xD8: # SET 3,B     | ---- | 2 8
            self.set_prefix("B", 3)
            cycles(8)
        elif prefix == 0xD9: # SET 3,C     | ---- | 2 8
            self.set_prefix("C", 3)
            cycles(8)
        elif prefix == 0xDA: # SET 3,D     | ---- | 2 8
            self.set_prefix("D", 3)
            cycles(8)
        elif prefix == 0xDB: # SET 3,E     | ---- | 2 8
            self.set_prefix("E", 3)
            cycles(8)
        elif prefix == 0xDC: # SET 3,H     | ---- | 2 8
            self.set_prefix("H", 3)
            cycles(8)
        elif prefix == 0xDD: # SET 3,L     | ---- | 2 8
            self.set_prefix("L", 3)
            cycles(8)
        elif prefix == 0xDE: # SET 3,(HL)  | ---- | 2 16
            self.set_prefix("(HL)", 3)
            cycles(16)
        elif prefix == 0xDF: # SET 3,A     | ---- | 2 8
            self.set_prefix("A", 3)
            cycles(8)
        elif prefix == 0xE0: # SET 4,B     | ---- | 2 8
            self.set_prefix("B", 4)
            cycles(8)
        elif prefix == 0xE1: # SET 4,C     | ---- | 2 8
            self.set_prefix("C", 4)
            cycles(8)
        elif prefix == 0xE2: # SET 4,D     | ---- | 2 8
            self.set_prefix("D", 4)
            cycles(8)
        elif prefix == 0xE3: # SET 4,E     | ---- | 2 8
            self.set_prefix("E", 4)
            cycles(8)
        elif prefix == 0xE4: # SET 4,H     | ---- | 2 8
            self.set_prefix("H", 4)
            cycles(8)
        elif prefix == 0xE5: # SET 4,L     | ---- | 2 8
            self.set_prefix("L", 4)
            cycles(8)
        elif prefix == 0xE6: # SET 4,(HL)  | ---- | 2 16
            self.set_prefix("(HL)", 4)
            cycles(16)
        elif prefix == 0xE7: # SET 4,A     | ---- | 2 8
            self.set_prefix("A", 4)
            cycles(8)
        elif prefix == 0xE8: # SET 5,B     | ---- | 2 8
            self.set_prefix("B", 5)
            cycles(8)
        elif prefix == 0xE9: # SET 5,C     | ---- | 2 8
            self.set_prefix("C", 5)
            cycles(8)
        elif prefix == 0xEA: # SET 5,D     | ---- | 2 8
            self.set_prefix("D", 5)
            cycles(8)
        elif prefix == 0xEB: # SET 5,E     | ---- | 2 8
            self.set_prefix("E", 5)
            cycles(8)
        elif prefix == 0xEC: # SET 5,H     | ---- | 2 8
            self.set_prefix("H", 5)
            cycles(8)
        elif prefix == 0xED: # SET 5,L     | ---- | 2 8
            self.set_prefix("L", 5)
            cycles(8)
        elif prefix == 0xEE: # SET 5,(HL)  | ---- | 2 16
            self.set_prefix("(HL)", 5)
            cycles(16)
        elif prefix == 0xEF: # SET 5,A     | ---- | 2 8
            self.set_prefix("A", 5)
            cycles(8)
        elif prefix == 0xF0: # SET 6,B     | ---- | 2 8
            self.set_prefix("B", 6)
            cycles(8)
        elif prefix == 0xF1: # SET 6,C     | ---- | 2 8
            self.set_prefix("C", 6)
            cycles(8)
        elif prefix == 0xF2: # SET 6,D     | ---- | 2 8
            self.set_prefix("D", 6)
            cycles(8)
        elif prefix == 0xF3: # SET 6,E     | ---- | 2 8
            self.set_prefix("E", 6)
            cycles(8)
        elif prefix == 0xF4: # SET 6,H     | ---- | 2 8
            self.set_prefix("H", 6)
            cycles(8)
        elif prefix == 0xF5: # SET 6,L     | ---- | 2 8
            self.set_prefix("L", 6)
            cycles(8)
        elif prefix == 0xF6: # SET 6,(HL)  | ---- | 2 16
            self.set_prefix("(HL)", 6)
            cycles(16)
        elif prefix == 0xF7: # SET 6,A     | ---- | 2 8
            self.set_prefix("A", 6)
            cycles(8)
        elif prefix == 0xF8: # SET 7,B     | ---- | 2 8
            self.set_prefix("B", 7)
            cycles(8)
        elif prefix == 0xF9: # SET 7,C     | ---- | 2 8
            self.set_prefix("C", 7)
            cycles(8)
        elif prefix == 0xFA: # SET 7,D     | ---- | 2 8
            self.set_prefix("D", 7)
            cycles(8)
        elif prefix == 0xFB: # SET 7,E     | ---- | 2 8
            self.set_prefix("E", 7)
            cycles(8)
        elif prefix == 0xFC: # SET 7,H     | ---- | 2 8
            self.set_prefix("H", 7)
            cycles(8)
        elif prefix == 0xFD: # SET 7,L     | ---- | 2 8
            self.set_prefix("L", 7)
            cycles(8)
        elif prefix == 0xFE: # SET 7,(HL)  | ---- | 2 16
            self.set_prefix("(HL)", 7)
            cycles(16)
        elif prefix == 0xFF: # SET 7,A     | --- | 2 8
            self.set_prefix("A", 7)
            cycles(8)
        else:
